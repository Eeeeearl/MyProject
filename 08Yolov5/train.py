...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Resume
    # 初始化开始训练的epoch和最好的结果
    # best_fitness是以[0.0, 0.0, 0.1, 0.9]为系数并乘以[精准度，召回率，mAP@0.5，mAP@0.5:0.95]再求和所得
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if ni <= nw:
                ...
                for j, x in enumerate(optimizer.param_groups):
                    ...
                    # 动量momentum也从0.9慢慢变到hyp['momentum']
                    if 'momentum' in x:
                        x['momentum'] = np.interp(ni, xi, [hyp['warmup_momentum'], hyp['momentum']])

            # Multi-scale
            # 设置多尺度训练，从imgsz * 0.5，imgsz * 1.5 + gs 随机选取尺寸
            if opt.multi_scale:
...
if __name__ == '__main__':
    ...
    # 模型配置文件，网络结构
    parser.add_argument('--cfg', type=str, default='', help='models/yolov5l.yaml path')
    # 配置训练数据路径，检测类别信息
    parser.add_argument('--data', type=str, default='data/sd_data.yaml', help='data.yaml path')
    # 配置一些学习率等相关的信息
    ...
    # Evolve hyperparameters (optional)
        ...
        meta = {'lr0': (1, 1e-5, 1e-1),             # 学习率          initial learning rate (SGD=1E-2, Adam=1E-3)
                'lrf': (1, 0.01, 1.0),              # 余弦退火超参数   final OneCycleLR learning rate (lr0 * lrf)
                ...
                # 设为0则表示不适用focal loss       focal loss gamma (efficientDet default gamma=1.5)
                'fl_gamma': (0, 0.0, 2.0),
                # 以下是一些数据增强的系数，包括颜色空间和图片空间
                'hsv_h': (1, 0.0, 0.1),             # 色调    image HSV-Hue augmentation (fraction)
                'hsv_s': (1, 0.0, 0.9),             # 饱和度    image HSV-Saturation augmentation (fraction)
                'hsv_v': (1, 0.0, 0.9),             # 明度    image HSV-Value augmentation (fraction)
                'degrees': (1, 0.0, 45.0),          # 旋转角度      image rotation (+/- deg)
                'translate': (1, 0.0, 0.9),         # 水平和垂直平移   image translation (+/- fraction)
                'scale': (1, 0.0, 0.9),             # 缩放        image scale (+/- gain)
                'shear': (1, 0.0, 10.0),            # 剪切        image shear (+/- deg)
                'perspective': (0, 0.0, 0.001),     # 透视变化参数    image perspective (+/- fraction), range 0-0.001
                'flipud': (1, 0.0, 1.0),            # 上下反转      image flip up-down (probability)
                'fliplr': (0, 0.0, 1.0),            # 左右翻转      image flip left-right (probability)
                'mosaic': (1, 0.0, 1.0),            # image mixup (probability)
                'mixup': (1, 0.0, 1.0)}             # mixup系数       image mixup (probability)
