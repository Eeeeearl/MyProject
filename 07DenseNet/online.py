...
class DenseNet_BC(nn.Module):
    def __init__(self, grow_rate=12, block_config=(6, 12, 24, 16), bn_size=4, theta=0.5, num_classes=10):
        super(DenseNet_BC, self).__init__()

        # 初始的卷积为filter:2倍的growth_rate
        num_init_feature = 2 * grow_rate
        ...
        for i, num_layers in enumerate(block_config):
            self.features.add_module('denseblock%d' % (i+1),
                                     _DenseBlock(num_layers, num_feature, bn_size, grow_rate))
        ...
        self.classifiter = nn.Linear(num_feature, num_classes)
    ...
    def forward(self, x):
        ...
        out = self.classifiter(out)
        return out


# DenseNet_BC for ImageNet
def DenseNet121():
    return DenseNet_BC(grow_rate=32, block_config=(6, 12, 24, 16), num_classes=1000)


def DenseNet169():
    return DenseNet_BC(grow_rate=32, block_config=(6, 12, 32, 32), num_classes=1000)


def DenseNet201():
    return DenseNet_BC(grow_rate=32, block_config=(6, 12, 48, 32), num_classes=1000)


def DenseNet161():
    return DenseNet_BC(grow_rate=48, block_config=(6, 12, 36, 24), num_classes=1000)


# DenseNet_BC for cifar
def densenet_BC_100():
    return DenseNet_BC(grow_rate=12, block_config=(16, 16, 16))
